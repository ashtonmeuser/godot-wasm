...
A simple example of loading a Wasm module, accessing its exported globals, and calling its exported functions. The Wasm module used is that generated by the [Wasm Create example](#wasm-create-assemblyscript). All logic for this Godot project exists in `Main.gd`.

### Wasm Create (AssemblyScript)

An example [AssemblyScript](https://www.assemblyscript.org) project which creates a simple Wasm module with three exported entities:
1. The `add` function accepting two `I64` values and returning an `I64` result.
1. The `global_const` global constant of type `I64`.
1. The `global_var` global mutable of type `F64`.

From the example directory (*examples/wasm-create*), run `npm run build` to build the Wasm module. This will create and populate a *build* directory containing the Wasm module, *add.wasm*, amongst other build artifacts. If you have the [Wasmer CLI](https://docs.wasmer.io/ecosystem/wasmer/getting-started) installed, run `wasmer inspect build/add.wasm` to view the Wasm module imports and exports.
