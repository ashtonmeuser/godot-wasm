...
void Wasm::_register_methods() {
  ...
  register_method("fire", &Wasm::fire);
}
...
godot_error Wasm::load(PoolByteArray bytecode) {
  ...
  ERR_FAIL_COND_V(!module, GODOT_ERR_COMPILATION_FAILED);

  // Instantiate
  wasm_extern_vec_t imports = WASM_EMPTY_VEC;
  instance = wasm_instance_new(store, module, &imports, NULL);
  ERR_FAIL_COND_V(!instance, GODOT_ERR_CANT_CREATE);
...
Variant Wasm::fire(String name, Array args) {
  ...
  switch (result.kind) {
    case WASM_I64: return Variant(result.of.i64);
    case WASM_F64: return Variant(result.of.f64);
