...
template <typename T>
wasi_encoded_strings encode_args(T args) {
...
wasm_trap_t *wasi_fd_write(void *env, const wasm_val_vec_t *args, wasm_val_vec_t *results) {
	...
	if (!wasm->has_permission("print")) {
		return godot_wasm::wasi_result(results, __WASI_ERRNO_ACCES, "Not permitted\0");
	}
...
wasm_func_t *wasi_callback(wasm_store_t *store, Wasm *wasm, callback_signature signature) {
	...
	for (auto &it : std::get<0>(signature)) {
		p_types->push_back(wasm_valtype_new(it));
	}
	for (auto &it : std::get<1>(signature)) {
		r_types->push_back(wasm_valtype_new(it));
	}
