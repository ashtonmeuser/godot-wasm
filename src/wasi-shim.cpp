...
namespace godot {
  namespace {
    ...
    wasm_trap_t* wasi_fd_write(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 4 || results->size != 1, "Invalid arguments fd_write", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      Wasm* wasm = (Wasm*)env;
      ...
      if (!wasm->has_permission("print")) return godot_wasm::wasi_result(results, __WASI_ERRNO_ACCES, "Not permitted\0");
    ...
    wasm_trap_t* wasi_proc_exit(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 1 || results->size != 0, "Invalid arguments proc_exit", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      Wasm* wasm = (Wasm*)env;
      if (!wasm->has_permission("exit")) return godot_wasm::wasi_result(results, __WASI_ERRNO_ACCES, "Not permitted\0");
    ...
    wasm_trap_t* wasi_args_sizes_get(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 2 || results->size != 1, "Invalid arguments args_sizes_get", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      Wasm* wasm = (Wasm*)env;
      wasm_memory_t* memory = wasm->get_memory().ptr()->get_memory();
      if (memory == NULL) return godot_wasm::wasi_result(results, __WASI_ERRNO_IO, "Invalid memory\0");
      if (!wasm->has_permission("args")) return godot_wasm::wasi_result(results, __WASI_ERRNO_ACCES, "Not permitted\0");
      byte_t* data = wasm_memory_data(memory);
    ...
    wasm_trap_t* wasi_args_get(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 2 || results->size != 1, "Invalid arguments args_get", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      Wasm* wasm = (Wasm*)env;
      wasm_memory_t* memory = wasm->get_memory().ptr()->get_memory();
      if (memory == NULL) return godot_wasm::wasi_result(results, __WASI_ERRNO_IO, "Invalid memory\0");
      if (!wasm->has_permission("args")) return godot_wasm::wasi_result(results, __WASI_ERRNO_ACCES, "Not permitted\0");
    ...
    wasm_trap_t* wasi_environ_sizes_get(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 2 || results->size != 1, "Invalid arguments environ_sizes_get", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      Wasm* wasm = (Wasm*)env;
      wasm_memory_t* memory = wasm->get_memory().ptr()->get_memory();
      if (memory == NULL) return godot_wasm::wasi_result(results, __WASI_ERRNO_IO, "Invalid memory\0");
      byte_t* data = wasm_memory_data(memory);
    ...
    wasm_trap_t* wasi_environ_get(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 2 || results->size != 1, "Invalid arguments environ_get", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      return godot_wasm::wasi_result(results);
    }

    // WASI random_get: [I32, I32] -> [I32]
    wasm_trap_t* wasi_random_get(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 2 || results->size != 1, "Invalid arguments random_get", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      Wasm* wasm = (Wasm*)env;
      wasm_memory_t* memory = wasm->get_memory().ptr()->get_memory();
      if (memory == NULL) return godot_wasm::wasi_result(results, __WASI_ERRNO_IO, "Invalid memory\0");
      if (!wasm->has_permission("random")) return godot_wasm::wasi_result(results, __WASI_ERRNO_ACCES, "Not permitted\0");
      byte_t* data = wasm_memory_data(memory);
    ...
    wasm_trap_t* wasi_clock_time_get(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
      FAIL_IF(args->size != 3 || results->size != 1, "Invalid arguments clock_time_get", godot_wasm::wasi_result(results, __WASI_ERRNO_INVAL, "Invalid arguments\0"));
      Wasm* wasm = (Wasm*)env;
      wasm_memory_t* memory = wasm->get_memory().ptr()->get_memory();
      if (memory == NULL) return godot_wasm::wasi_result(results, __WASI_ERRNO_IO, "Invalid memory\0");
      if (!wasm->has_permission("time")) return godot_wasm::wasi_result(results, __WASI_ERRNO_ACCES, "Not permitted\0");
      byte_t* data = wasm_memory_data(memory);
  ...
  namespace godot_wasm {
    wasm_trap_t* wasi_result(wasm_val_vec_t* results, int32_t value, const char* message) {
      if (results->size > 0) {
