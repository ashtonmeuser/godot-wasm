...
struct ContextFuncImport : public ContextExtern {
	...
	ContextFuncImport(uint16_t i, const wasm_functype_t *func_type) :
		...
		for (uint16_t i = 0; i < func_results->size; i++) {
			results.push_back(wasm_valtype_kind(func_results->data[i]));
		}
	}
};
...
struct ContextFuncExport : public ContextExtern {
	...
	ContextFuncExport(uint16_t i, const wasm_functype_t *func_type) :
		...
		const wasm_valtype_vec_t *func_results = wasm_functype_results(func_type);
		for (uint16_t i = 0; i < func_params->size; i++) {
			params.push_back(wasm_valtype_kind(func_params->data[i]));
		}
...
Variant decode_variant(wasm_val_t value) {
	switch (value.kind) {
		case WASM_I32:
			return Variant(value.of.i32);
		case WASM_I64:
			return Variant(value.of.i64);
		case WASM_F32:
			return Variant(value.of.f32);
		case WASM_F64:
			return Variant(value.of.f64);
		default:
			FAIL("Unsupported Wasm type", NULL_VARIANT);
	}
}
...
Array get_extern_signature(const wasm_module_t *module, uint16_t index, bool import) {
	...
	switch (wasm_externtype_kind(type)) {
		case WASM_EXTERN_FUNC: {
			...
			for (uint16_t i = 0; i < func_params->size; i++) {
				param_types.append(get_value_type(wasm_valtype_kind(func_params->data[i])));
			}
			for (uint16_t i = 0; i < func_results->size; i++) {
				result_types.append(get_value_type(wasm_valtype_kind(func_results->data[i])));
			}
		...
		default:
			FAIL("Unsupported extern type", Array());
	}
}
...
wasm_trap_t *callback_wrapper(void *env, const wasm_val_vec_t *args, wasm_val_vec_t *results) {
	...
	for (uint16_t i = 0; i < args->size; i++) {
		params.push_back(decode_variant(args->data[i]));
	}
	...
	if (error) {
		FAIL("Extracting import function results failed", trap("Extracting import function results failed\0"));
	}
	return NULL;
}
} //namespace
...
godot_error Wasm::instantiate(const Dictionary import_map) {
	...
	for (auto &it : extern_map) {
		extern_list.push_back(it.second); // Maps iterate over sorted keys
	}
...
Dictionary Wasm::inspect() const {
	...
	for (const auto &tuple : import_funcs) {
		import_func_sigs[tuple.first] = get_extern_signature(module, tuple.second.index, true);
	}
	for (const auto &tuple : export_globals) {
		export_global_sigs[tuple.first] = get_extern_signature(module, tuple.second.index, false);
	}
	for (const auto &tuple : export_funcs) {
		export_func_sigs[tuple.first] = get_extern_signature(module, tuple.second.index, false);
	}
	...
	if (memory_context != NULL) {
		dict_memory["import"] = memory_context->import;
	}
...
Variant Wasm::function(String name, Array args) const {
	...
	// Extract result(s)
	if (context.return_count == 0) {
		return NULL_VARIANT;
	}
	if (context.return_count == 1) {
		return decode_variant(f_results.data[0]);
	}
	Array results = Array();
	for (uint16_t i = 0; i < context.return_count; i++) {
		results.append(decode_variant(f_results.data[i]));
	}
	return results;
}
...
godot_error Wasm::map_names() {
	...
	for (uint16_t i = 0; i < imports.size; i++) {
		...
		const String key = decode_name(wasm_importtype_module(curr_data)) + "." + decode_name(wasm_importtype_name(curr_data));
		switch (kind) {
			case WASM_EXTERN_FUNC: {
	...
	for (uint16_t i = 0; i < exports.size; i++) {
		...
		switch (kind) {
			...
			case WASM_EXTERN_MEMORY:
				if (memory_context == NULL) {
					memory_context = new godot_wasm::ContextMemory(i, false); // Favour import memory
				}
				break;
			case WASM_EXTERN_TABLE:
